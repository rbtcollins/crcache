...
"""'Arguments' for cr_cache.

This is a small typed arguments concept - which is perhaps obsoleted by
argparse in Python 2.7, but for now is an extension used with optparse.

The code in this module contains the AbstractArgument base class. Individual
argument types are present in e.g. cr_cache.arguments.string.

See cr_cache.commands.Command for usage of Arguments.

Plugins and extensions wanting to add argument types should either define them
internally or install into cr_cache.arguments as somename (perhaps by
extending the cr_cache.arguments __path__ to include a directory
