...
"""The crcache command abstraction.

Interesting things here:
run_argv: the CLI entry point.
...
def _find_command(cmd_name):
    ...
    modname = "cr_cache.commands.%s" % cmd_name
    try:
...
class Command(object):
    ...
    :ivar store_factory: a factory which is used to obtain the data store.
        The default factory is suitable for use in the command line tool.

    Commands declare that they accept/need/emit:
    :ivar args: A list of cr_cache.arguments.AbstractArgument instances.
    ...
    def __init__(self, ui, store_factory=None):
        """Create a Command object with a ui and store factory."""
        self.ui = ui
        if store_factory is None:
            store_factory = local.Store
